...
volatile uint16_t USB_DeviceStatus;
volatile uint8_t  USB_DeviceAddress;
volatile uint8_t  USB_Configuration;
volatile uint32_t USB_EndPointMask;
volatile uint32_t USB_EndPointHalt;
volatile uint32_t USB_EndPointStall;                         /* EP must stay stalled */
volatile uint8_t  USB_NumInterfaces;
volatile uint8_t  USB_AltSetting[USB_IF_NUM];

union _uEP0 uEP0 __attribute__((aligned(4)));
  ...
  switch (SetupPacket.bmRequestType.BM.Recipient) {
    ...
    case REQUEST_TO_ENDPOINT:
      ...
      if (((USB_Configuration != 0) || ((n & 0x0F) == 0)) && (USB_EndPointMask & m)) {
    	uEP0.EP0Buf16[0] = (USB_EndPointHalt & m) ? 1 : 0;
        EP0Data.pData = uEP0.EP0Buf;
      } else {
        return (FALSE);
      }
              ...
              if ((SetupPacket.wIndex.WB.L == USB_CDC_CIF_NUM)  ||       /* IF number correct? */
                ...
                switch (SetupPacket.bRequest) {
                  case CDC_SEND_ENCAPSULATED_COMMAND:
                    EP0Data.pData = uEP0.EP0Buf;                              /* data to be received, see USB_EVT_OUT */
                    goto setup_class_ok;
                  case CDC_GET_ENCAPSULATED_RESPONSE:
                    if (CDC_GetEncapsulatedResponse()) {
                      EP0Data.pData = uEP0.EP0Buf;                            /* point to data to be sent */
                      USB_DataInStage();                                 /* send requested data */
                      goto setup_class_ok;
                    }
                    break;
