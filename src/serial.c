...
typedef struct __SER_BUF_T {
  volatile unsigned char data[SER_BUF_SIZE];
  volatile unsigned int wrIdx;
  volatile unsigned int rdIdx;
} SER_BUF_T;

volatile unsigned long          ser_txRestart;                  // NZ if TX restart is required
volatile unsigned short         ser_lineState;                  // ((msr << 8) | (lsr))
