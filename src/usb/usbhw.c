...
void USB_Init(void)
{
	...
	LPC_USB->USBClkCtrl = 0x12; /* / 1A / Dev, PortSel, AHB clock enable */
...
void USB_Reset(void)
{
	...
	LPC_USB->USBDevIntEn = DEV_STAT_INT | EP_SLOW_INT
			| (USB_SOF_EVENT ? FRAME_INT : 0) | (USB_ERROR_EVENT ? ERR_INT : 0);
...
void USB_IRQHandler(void)
{
	...
	if (disr & EP_SLOW_INT)
	{
		...
		for (n = 0, episrCur = 1; n < USB_EP_NUM; n++, episrCur <<= 1)
		{ /* Check All Endpoints */
			if (episr == 0)
	...
	if (LPC_USB->USBDMAIntSt & 0x00000001)
	...
	if (LPC_USB->USBDMAIntSt & 0x00000002)
	...
	if (LPC_USB->USBDMAIntSt & 0x00000004)
