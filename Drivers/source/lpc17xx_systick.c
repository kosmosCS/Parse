...
volatile U32 ST_tms    = 0;
volatile U32 ST_timer  = 0;
volatile U32 ST_timer2 = 0;
U32	ST_stamp;
...
void SYSTICK_InternalMyInit(uint32_t time)
{
	if(time >= (1<<24))
		//Error loop
		while(1);
...
void SysTick_SetStamp(void)
{
	ST_stamp = SysTick->VAL;
}

U32 SysTick_GetDelta(void)
{
	return ST_stamp - SysTick->VAL;
}
